...
const config = {
    session: {
        server: "host=localhost&port=2424&username=root&password=jjanmo&db=o2"
    }
};
 ~~ : new OrientStore(config.session);
*/
...
//-> 이렇게 설정하면 database 'o2'에 sessions라는 스키마가 생기고 그곳에 session정보가 저장됨
...
app.get("/auth/logout", function (req, res) {
    ...
    //-> database에 반영이 안된 채로 redirect되는 경우를 막을 수 있음
    //-> redirect하는 경우에만 사용하면 됨
    req.session.save(function () {
        res.redirect("/welcome");
    });
